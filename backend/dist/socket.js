...
const setupSocket = (httpServer) => {
    ...
    io.on('connection', (socket) => {
        ...
        // --- JOIN ROOM ---
        socket.on('room:join', (roomId, userId) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                // 2. Check Bans
                const ban = yield db_1.default.roomBan.findUnique({
                ...
                if (room.participantCount >= room.maxParticipants) {
                    ...
                    const existing = yield db_1.default.roomParticipant.findUnique({
                        where: { userId_roomId: { userId, roomId } }
                    });
                    if (!existing) {
                ...
                const participantCount = yield db_1.default.roomParticipant.count({ where: { roomId } });
                let role = 'MEMBER';
                let status = 'LISTENER';
                // If owner, force role
                ...
                const participant = yield db_1.default.roomParticipant.upsert({
                    where: { userId_roomId: { userId, roomId } },
        ...
        // --- LEAVE ROOM ---
        socket.on('room:leave', (roomId, userId) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                socket.leave(roomId);
                // Update DB
                yield db_1.default.roomParticipant.deleteMany({
                    where: { userId, roomId }
                });
                const room = yield db_1.default.room.update({
                    where: { id: roomId },
                ...
                if (room.participantCount <= 0) {
                    ...
                    const timer = setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
                        const r = yield db_1.default.room.findUnique({ where: { id: roomId } });
        ...
        socket.on('room:kick', (roomId, targetUserId, requesterId) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                // Verify permissions
                const requester = yield db_1.default.roomParticipant.findUnique({ where: { userId_roomId: { userId: requesterId, roomId } } });
                const target = yield db_1.default.roomParticipant.findUnique({ where: { userId_roomId: { userId: targetUserId, roomId } } });
                const room = yield db_1.default.room.findUnique({ where: { id: roomId } });
                if (!requester || !target || !room)
                    return;
                // Logic: Owner can kick anyone (except self). Co-Owner can kick Members.
                let canKick = false;
                if (requester.role === 'OWNER' && target.userId !== requesterId)
                    canKick = true;
                if (requester.role === 'CO_OWNER' && target.role === 'MEMBER')
                    canKick = true;
                if (!canKick) {
                ...
                // Log action
                yield db_1.default.moderationLog.create({
                    data: {
                        roomId,
                        actorId: requesterId,
        ...
        // --- SPEAKER/LISTENER ---
        socket.on('room:raise-hand', (roomId, userId) => __awaiter(void 0, void 0, void 0, function* () {
            yield db_1.default.roomParticipant.update({
                where: { userId_roomId: { userId, roomId } },
                data: { wantsToSpeak: true }
            });
            io.to(roomId).emit('room:hand-raised', { userId });
        }));
        socket.on('room:grant-speaker', (roomId, targetUserId, requesterId) => __awaiter(void 0, void 0, void 0, function* () {
            // Verify requester is OWNER/CO_OWNER
            const requester = yield db_1.default.roomParticipant.findUnique({ where: { userId_roomId: { userId: requesterId, roomId } } });
            if (!requester || (requester.role !== 'OWNER' && requester.role !== 'CO_OWNER'))
                return;
            yield db_1.default.roomParticipant.update({
        ...
        socket.on('room:revoke-speaker', (roomId, targetUserId, requesterId) => __awaiter(void 0, void 0, void 0, function* () {
            // Verify requester is OWNER/CO_OWNER
            const requester = yield db_1.default.roomParticipant.findUnique({ where: { userId_roomId: { userId: requesterId, roomId } } });
            if (!requester || (requester.role !== 'OWNER' && requester.role !== 'CO_OWNER'))
                return;
